...
export function toSyncJobDocument(row: CorpusSyncJobRow): CorpusSyncJobDocument {
	return {
		...
		setsFetched: row.setsFetched,
		setsUnchanged: row.setsUnchanged,
		setsFlaggedMissing: row.setsFlaggedMissing,
...
export function readCorpusStatus(db: GloomDatabase): CorpusStatusDocument {
	...
	return {
		...
		sets: totals.sets,
		setsWithoutReleaseDate: totals.setsWithoutReleaseDate,
