...
**Six new frozen decisions** came out of that review: the runtime (`01m04je8az`), the push
rule (`01m04je8wa`), card-grain matching (`01m04je9e7`), the completion denominator
(`01m04jea06`), the condition ladder (`01m04k28t8`) and the backfill (`01m04mt0sv`). Three
of them supersede frozen decisions that were built on facts which did not survive contact
with primary sources — **Bun 1.4 was never published**, eBay's condition vocabulary is not
the one the map recorded, and card condition is not obtainable from a search result at all.

This revision also adds the artifacts the review found missing — schema, API surface,
configuration with defaults, and conventions for time, money and identity — and corrects
the variant model, which needs **five axes** rather than the two earlier drafts described.

The review is recorded as *Adversarial reader review — Gloom Watch build-ready spec*
(`01m04hm3vs`). Every external claim below has been checked against a primary source except
those explicitly listed under *Facts to verify at build time*.
...
51. As the owner, I want backups encrypted with a key the server cannot read, so a
    compromised box cannot open its own backup history.
...
Column lists are the decision-relevant ones, not exhaustive DDL. Every table carries
`created_at` and `updated_at` as UTC epoch milliseconds.

**`cards`** — one language-specific printed card record. `card_id` TEXT PK
(`{setId}-{localId}` within a language namespace). Carries `language`, `set_id`,
`set_name`, `local_id`, `name`, `rarity`, `provenance` (`tcgdex` | `manual`),
`missing_upstream` INTEGER, `last_synced_at`, `image` BLOB, `image_hash` TEXT.

**Images hang off the card, not the variant.** TCGdex ships one image per card record
(~475 for this line) and has no per-variant art; ~765 variants therefore share ~475
images. A binder cell renders its card's image.

**`variants`** — the unit of collecting. PK **`(card_id, variant_id)`**, FK to `cards`.
Carries `finish`, `subtype` (nullable), `stamps` TEXT (canonicalised, sorted, JSON array
— **never a scalar**), `foil` (nullable), `size`, `raw_attributes` TEXT (the upstream
object verbatim, since the vocabulary is demonstrably not tightly controlled),
`provenance`, `missing_upstream`, `priority` INTEGER, `last_synced_at`.

**`variant_id` is not always a hash.** The literal sentinel string `"generated"` appears
as a `variant_id` on real cards. Ingest must treat it as an opaque token and never assume
a hash shape or a fixed length.

**The collision is severe, which is why the compound key is not optional.** In a
159-card sample, 256 variant records carried only 21 distinct `variant_id` values, and one
of them — `endfynwn4n10gzq` — was shared by **90 different cards**. Three different Base
Set cards return byte-identical `variant_id`s for their unlimited-holo printing. Keying on
`variant_id` alone would collapse ninety cards into one row.

**Manual rows get a synthetic `card_id` in a reserved namespace** — `manual:{uuid}` — and
a synthetic `variant_id`. This is required, not cosmetic: a Korean clone of a Japanese
card has no upstream `card_id` to inherit, and copying the source's identity would
collide with it on the upsert key. The reserved prefix also guarantees a future TCGdex
addition of those languages can never collide with a hand-made row.

**`copies`** — one physical card. `id` TEXT PK (client-generated UUID, see Outbox), FK
`(card_id, variant_id)`. Fields as ruled in `01m03xa7ty`, below.

**`copy_photos`** — `id`, `copy_id`, `image` BLOB (webp), `width`, `height`, `bytes`,
`created_at`.

**`listings`** — `item_id` TEXT PK, `marketplace`, `title`, `price_minor` INTEGER,
`currency` TEXT, `buying_option`, `condition_id`, `item_web_url`, `seller_hash`,
`observed_at`, `payload` TEXT (purged at 90 days), `payload_purged_at`.

**`listing_matches`** — `item_id` PK, `match_grain` (`variant` | `card` | `none`),
`card_id`, `variant_id` (**only** when grain is `variant`), `candidates` TEXT,
`language`, `confidence` REAL, `matcher_version` TEXT, `is_lot`, `filter_verdict`,
`filter_reason`, `parsed_grader`, `parsed_grade`.

**`seen_items`** — `item_id` PK, `first_seen_at`. **Never purged.** This is the `itemId`
dedupe set and it must outlive the 90-day payload purge, or relists re-notify silently at
day 91. It holds no eBay content — only an opaque identifier and a timestamp.

**`aliases`** — `id`, `pattern` TEXT, `card_id`, `language`, `created_at`. Owner-authored.

**`exclusions`** — `card_id` PK. The manual exclusion list for name-sweep false hits.
**Owner-curated with no upstream source**, so it carries the same re-import protection and
export treatment as manual variants; a re-import must never discard it.

**`notifications`** — `id`, `kind` (`instant` | `digest` | `gap_recovery`), `sent_at`,
`item_ids` TEXT, `payload_bytes`, `endpoint_response`.

**`push_subscriptions`** — `endpoint` PK, `p256dh`, `auth`, `created_at`,
`last_success_at`, `last_failure_at`, `failure_count`. **Re-subscribing replaces the row
for the same endpoint.** Because iOS never fires `pushsubscriptionchange` and Apple's
endpoint may return 201 for dead subscriptions, prune only on an explicit 404/410 or on
the owner re-subscribing; otherwise fan-out to stale endpoints is the default outcome.

**`settings`** — `key` PK, `value` TEXT. See Configuration.

**`scan_state`** — `marketplace` PK, `last_scanned_at`, `last_success_at`,
`consecutive_failures`. **Per-marketplace, not global**: DE and AU run every fourth cycle,
so a single global cursor would advance past the listings they sat out and lose most of
them.

**Indexes:** `listings(observed_at)`, `listings(payload_purged_at)`,
`listing_matches(variant_id)`, `listing_matches(card_id)`, `copies(card_id, variant_id)`,
`copies(status)`, `variants(card_id)`.
...
**Identifiers.** Rows the client can create — copies, photos, manual variants — use
**client-generated UUIDs**, so an outbox replay is idempotent.

### Configuration

Every tunable lives in the `settings` table and is editable on a **settings screen**,
which is a v1 surface. Values below are **starting points to be tuned against real data**,
not rulings.
...
~25,000 active Gloom listings in the US category alone. The backfill pages through active
inventory, matches it, seeds `seen_items`, and **does not notify**; then each cursor is set
to now and the forward scan is armed.
...
**Raw payloads are retained 90 days** with the match, confidence and **matcher version**,
so an improved matcher can be re-run over retained listings and compared. Beyond 90 days
the payload is gone and re-runs are impossible — a **rolling 90-day evaluation window**,
which is the cost of the intermediate-copies clause. `seen_items` survives independently.
...
for iOS 16.4–18.3. **The server stores which transport each subscription supports** and
sends the matching payload shape; it does not send both.
...
**Dedupe:** a `seen_items` hit never re-notifies; and a ~30-day relist guard on hashed
...
**The badge is a lagging indicator.** It counts unseen qualifying listings and refreshes
only when a notification is shown or the app is opened. **Seen** means the listing has
appeared in the feed while the app was foregrounded.
...
JSON over HTTP, same-origin, no auth (see Deployment). Hono RPC gives the client
end-to-end types.

```
GET    /api/binder                    variants + ownership + completion, one cacheable doc
GET    /api/variants/:cardId/:variantId
POST   /api/copies                    body carries client-generated id
PATCH  /api/copies/:id
POST   /api/copies/:id/photos         multipart; resized server-side
DELETE /api/copies/:id/photos/:photoId
PATCH  /api/variants/:cardId/:variantId/priority
POST   /api/variants                  manual entry (clone or blank)
GET    /api/listings                  filterable feed
GET    /api/listings/:itemId          the notification tap target
GET    /api/queue                     confirm queue
POST   /api/queue/:itemId/resolve     confirm | pick-variant | not-a-match
GET    /api/lots
POST   /api/corpus/sync               manual corpus refresh
GET    /api/health                    scan, backup, corpus, outbox state
GET    /api/settings   PATCH /api/settings
POST   /api/push/subscribe   DELETE /api/push/subscribe
GET    /api/images/:cardId            corpus image BLOB, CacheFirst
GET    /api/photos/:photoId           owner photo BLOB
POST   /api/export                    generate CSV + JSON
```
...
**Encryption and dedup cannot both be had naively.** `age` encrypts each file under a
fresh ephemeral key, so two encryptions of a 125 MB database dedupe to nothing.
**Resolution: use `restic` against both destinations**, which chunks and dedups plaintext
and manages its own crypto — with the repository password held **off the box** in a
password manager and supplied to the backup job from the environment file. This keeps
dedup and keeps the operator's key custody off-machine; it gives up the strict
write-only property that a public-key-only recipient would have provided. That trade is
made deliberately: dedup is what makes the debounced cadence affordable.
...
**Export rows join on `(card_id, variant_id)`**, not on human-readable columns, so
re-import against a freshly ingested corpus is exact. Manual rows carry their
`manual:{uuid}` identity, so they restore without collision.
...
**The backfill has its own tests**: it pages to exhaustion; it seeds `seen_items` for every
...
**Retention jobs get their own tests**: the 90-day payload purge leaves `seen_items` and
...
9. **Backfill run to completion**, `seen_items` seeded, cursors set to now, forward scan
   armed
